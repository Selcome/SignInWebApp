...
DataProvider.prototype.group = function (keys, condition, initial, reduce, finalize, command, options, callback) {
    this.getCollection(function (err, collection) {
        if (err) callback(err);
        else {
            collection.group(keys, condition, initial, reduce, finalize, command, options,function(err, result) {
                callback(err, result);
            });
        }
    });
...
DataProvider.prototype.findAndModify = function (query, sort, doc,options, callback) {
    this.getCollection(function (err, collection) {
        if (err) callback(err);
        else {
